...
(() => {
  ...
  function advance(event) {
    ...
    if (s.guided && event === expected[s.stage]) {
      s.stage++;
  ...
  function render() {
    ...
    root.innerHTML = `${header()}<main class="experience"><div class="experience-caption"><p>웹은 그대로. <strong>Keeper가 업무를 배웁니다.</strong></p><span>데이터·도구·Skill이 연결된 합성 체험</span></div><div class="workspace"><section class="browser" aria-label="합성 웹앱"><div class="browser-bar"><div class="lights" aria-hidden="true"><i></i><i></i><i></i></div><span class="address">◈ ${s.site === "minio" ? "minio.demo / browser" : "wafersight.demo / dashboard"}</span><span class="browser-status">${s.ready ? "● 도구 준비 완료" : s.discovered ? "◐ 후보 시험 필요" : "○ 첫 방문 · 도구 없음"}</span></div>${minio()}</section>${keeper()}</div></main>`;
    if (s.guided) {
